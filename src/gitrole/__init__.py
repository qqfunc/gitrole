...
class GitRole:
        ...
        if config_path is None:
            ...
            if not self.config_path.is_file():
                if self.config_path.is_dir():
                    sys.exit(f"{self.config_path} is a directory")
                sys.exit(f"Configuration file not found: {self.config_path}")
    ...
    def apply_role(self) -> None:
        ...
        if not isinstance(config, dict):
            sys.exit(f"Invalid configuration for role '{self.role}'.")

        for key, value in config.items():
            if not isinstance(value, str):
                sys.exit(f"Invalid value for '{key}' in role '{self.role}'.")
